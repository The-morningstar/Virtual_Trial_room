...
app = Flask(__name__)   #app is the instance of flask class     The __name__ parameter is a special Python variable that refers
                        # to the name of the current module. When __name__ is passed to the Flask constructor, it helps Flask determine the root path of the application, which is necessary for locating resources such as templates and static files.
CORS(app)               #The CORS(app) statement is a code snippet that enables Cross-Origin Resource Sharing (CORS)
                        # support in a Flask application
                        #nabling CORS is useful when you have a client-side application
                        #(e.g., a web page with JavaScript) making HTTP requests to the Flask backend from a different domain.
                        # It helps in avoiding cross-origin request restrictions imposed by modern web browsers.


@app.route('/')                             #@app.route('/') is a decorator that binds the URL path '/' to the index function.
def index():                                # In this case, the '/' path represents the root URL of the Flask application.
    return render_template('index.html')    # Whenever a user visits the root URL, Flask will call the index function.
...
def plot():
    return render_template('shirt.html')  # response returned by plot() is render_template('shirt.html) .
                                          # the function  render the html template and return it as a response ti the call


@app.route('/pant.html')
def ploty():                              #ploty() is a function that handles requeest made to '/pant.html' url
    return render_template('pant.html')


@app.route('/predict', methods=['GET', 'POST'])
def predict():
    shirtno = int(request.form["shirt"])     # retrieves the value of the form field with the name "shirt" from the incoming request.
                                             # It assumes that the form data is being sent with a POST request and contains a field named "shirt".
                                             # It then converts the retrieved value to an integer and assigns it to the shirtno variable.
    ...
    while True:
        ...
        if ih == 3:
            ...
            ret, orig_masks_inv = cv2.threshold(shirtgray, 200, 255,
                                                    cv2.THRESH_BINARY)  # there may be some issues with image threshold...depending on the color/contrast of image
            orig_masks = cv2.bitwise_not(orig_masks_inv)

        else:
            shirtgray = cv2.cvtColor(imgshirt, cv2.COLOR_BGR2GRAY)  # grayscale conversion
            ret, orig_masks = cv2.threshold(shirtgray, 0, 255,
                                                cv2.THRESH_BINARY)  # there may be some issues with image threshold...depending on the color/contrast of image
        ...
        if i == 1:
            ret, orig_mask = cv2.threshold(pantgray, 100, 255,
                                               cv2.THRESH_BINARY)  # there may be some issues with image threshold...depending on the color/contrast of image
        ...
        for (x, y, w, h) in faces:
            ...
            pant = cv2.resize(imgpant, (pantWidth, pantHeight),
                                  interpolation=cv2.INTER_AREA) # resize all,the masks you made,the originla image,everything
            ...
            """
            # Re-size the original image and the masks to the shirt sizes
            shirt = cv2.resize(imgshirt, (shirtWidth, shirtHeight),
                                   interpolation=cv2.INTER_AREA)  # resize all,the masks you made,the originla image,everything
